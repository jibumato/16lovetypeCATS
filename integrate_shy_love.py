#!/usr/bin/env python3
"""Integrate mbti-shy-love-en.html into the entire site."""
...
RANKING_NEW = (
    RANKING_OLD
    + '\n      <a href="/mbti-shy-love-en.html" style="display:block;background:var(--bg);'
    'border:1.5px solid var(--beige);border-radius:12px;padding:10px 14px;'
    'font-size:13px;font-weight:700;margin-top:6px;text-decoration:none">'
    'MBTI Types That Pull Away When They Like You</a></div>'
)

for fname in RANKING_FILES:
    path = os.path.join(BASE, fname)
    text = open(path, encoding="utf-8").read()
    if "mbti-shy-love-en" in text:
        print(f"SKIP: {fname}")
        continue
...
TYPE_NEW_PLAIN = (
    ...
    '</li>\n              '
    '<li style="padding:10px 0;font-size:14px">'
    '<a href="/mbti-shy-love-en.html">MBTI Types That Pull Away When They Like You</a>'
    '</li></ul></div></div>'
)
...
SHY_LOVE_LINK = (
    ...
    'border:1.5px solid var(--beige);border-radius:12px;padding:10px 14px;'
    'font-size:13px;font-weight:700;margin-top:6px;text-decoration:none">'
    'MBTI Types That Pull Away When They Like You</a>'
)
...
for fname in GUIDE_FILES:
    ...
    if "mbti-shy-love-en" in text:
        print(f"SKIP: {fname}")
        continue
    if APPROACH_LINK not in text:
        print(f"WARN (approach pattern not found): {fname}")
        continue
    text = text.replace(APPROACH_LINK, APPROACH_LINK + SHY_LOVE_LINK, 1)
    open(path, "w", encoding="utf-8").write(text)
...
if "mbti-shy-love-en" not in sitemap:
    ...
    sitemap = sitemap.replace(APPROACH_ENTRY, APPROACH_ENTRY + "\n" + SHY_LOVE_ENTRY, 1)
...
if "mbti-shy-love-en" not in index:
    APPROACH_LI = '<li><a href="/mbti-approach-en.html">How to Approach Each MBTI Type: The Best Strategy to Win Their Heart (English)</a></li>'
    SHY_LOVE_LI = '\n<li><a href="/mbti-shy-love-en.html">MBTI Types That Pull Away When They Like You: Shy Love Explained (English)</a></li>'
    index = index.replace(APPROACH_LI, APPROACH_LI + SHY_LOVE_LI, 1)
