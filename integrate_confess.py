#!/usr/bin/env python3
"""Integrate mbti-confess-en.html into the entire site."""
...
for fname in GUIDE_FILES:
    ...
    if "mbti-confess-en" in text:
        print(f"SKIP: {fname}")
        continue
    if ACTS_COLD_LINK not in text:
        print(f"WARN (acts-cold pattern not found): {fname}")
        continue
    text = text.replace(ACTS_COLD_LINK, ACTS_COLD_LINK + CONFESS_LINK, 1)
    open(path, "w", encoding="utf-8").write(text)
    print(f"OK guide: {fname}")

# ─── 4. mbti-kokuhaku.html hreflang + multilingual card ──────────────────────
src_path = os.path.join(BASE, "mbti-kokuhaku.html")
...
if "mbti-confess-en" not in src_text:
    RELATED_H2 = '<h2 style="font-size:16px">📰 関連記事</h2>'
    MULTI_CARD = (
        ...
        '      <li><a href="/mbti-confess-en.html">How to Confess to Each MBTI Type (English)</a></li>\n'
        '    </ul>\n'
        '  </div>\n\n  '
        + RELATED_H2
    )
    src_text = src_text.replace(RELATED_H2, MULTI_CARD, 1)
    print("OK multilingual card: mbti-kokuhaku.html")
else:
...
if "mbti-confess-en" not in sitemap:
    ...
    sitemap = sitemap.replace(ACTS_COLD_ENTRY, ACTS_COLD_ENTRY + "\n" + CONFESS_ENTRY, 1)
...
if "mbti-confess-en" not in index:
    ACTS_COLD_LI = '<li><a href="/mbti-acts-cold-en.html">MBTI Types That Act Cold When They Like You: Why &amp; How to Reach Them (English)</a></li>'
    CONFESS_LI = '\n<li><a href="/mbti-confess-en.html">How to Confess to Each MBTI Type: The Words &amp; Timing That Actually Work (English)</a></li>'
    index = index.replace(ACTS_COLD_LI, ACTS_COLD_LI + CONFESS_LI, 1)
